...
func (h *Handler) overview(w http.ResponseWriter, r *http.Request) {
	...
	h.db.Model(&models.Link{}).Select("COALESCE(SUM(clicks),0)").Scan(&totalClicks)
	...
	top := func(col string) []statKV {
		...
		h.db.Model(&models.LinkEvent{}).
			...
			Where("created_at >= ? AND "+col+" <> ''", since30).
	...
	writeJSON(w, http.StatusOK, map[string]any{
		...
		"emails":       count(&models.Email{}),
		"unread":       count(&models.Email{}, "read = ?", false),
		"tokens":       count(&models.Token{}),
		"totalClicks":  totalClicks,
		"clicks7d":     count(&models.LinkEvent{}, "created_at >= ?", since7),
		"clicks30d":    count(&models.LinkEvent{}, "created_at >= ?", since30),
		"series":       series,
		"topLinks":     topLinks,
		"devices":      top("device"),
